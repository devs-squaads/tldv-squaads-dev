...
- [ ] 9.3 Manual walkthrough (UI-visual exempt per AGENTS.md, per plan.md Testing Strategy "Exempt" row): bell badge count, admin-only page guard, 3 recipient modes, passive rejection surfaces (both recipient paths), real SMTP send with configured env. [Originally not executed — no browser in that environment; checklist handed to the orchestrator/user in the PR6 apply report. **Update (same session, after this note was written):** a human has since manually tested live: the member-role 3-recipient-mode share-creation flow (all-participants/subset/manual-email), a real SMTP email delivery to a live Gmail inbox (confirmed received, link worked), and the UI fixes shipped alongside this feature (`InfoModal`/`ConfirmModal` replacing native `alert`/`confirm`, the `<select>` dropdown theming fix, resolved emails/meeting names replacing raw ids). **Still not tested live:** admin approve/reject actions, and an admin-role Owner sharing directly (no Share Request). Checkbox stays unchecked — the walkthrough is genuinely partial, not complete.]

...
| `DEV_AUTH_BYPASS_EMAIL`                                       | Email ya autorizado (`authorized_accounts` o `SUPER_ADMIN_EMAILS`) que habilita un login sin Google vía `CredentialsProvider`. Solo tiene efecto si `VERCEL_ENV !== "production"` (o está ausente, como en `next dev` local); nunca se activa en Production aunque la variable esté cargada. Ver `isDevAuthBypassEnabled` en `apps/web/src/auth.ts`. |
...
| `GEMINI_LIVE_MODEL`                                           | Opcional. Modelo de Gemini Live para la voz. Default verificado: `gemini-3.8-live`. |
...
- Por voz solo se pueden ejecutar las tools de lectura (`search_meetings`, `get_meeting_detail`,
  `get_system_status`); el puente `POST /api/chat/voice/tool` responde `403` para cualquier otra.
- La conversación se persiste en el historial del chat (`/api/chat/history`) y sigue ahí tras
  recargar. Hay rate limit por usuario en el minteo (`429`) y un tope de duración configurable que
  cierra la sesión con aviso visible.
